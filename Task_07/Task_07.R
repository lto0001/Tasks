...
#a plot for each allele's frequency overtime 
plot(1, 1, type="n", xlim=c(1998, 2013), ylim=c(0, 1))
s <- apply(overallFreq, 2, function(x) lines(overallFreq[, 1], x, col=rgb(0, 0, 0, 0.01)))

#a plot of each allele's rescaled frequency (observed - initial freq) over time 
rescaleFreq <- apply(overallFreq[, 3:nol(overallFreq)], 2, function(x)x-x[1])
plot(1, 1, type="n", xlim=c(1998, 2013), ylim=c(-0.25, 0.25))
s <- apply(rescaleFreq, 2, function(x) lines(overallFreq[,1], x, col=rbg(0, 0, 0, 0.01)))
